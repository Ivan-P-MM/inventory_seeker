...
export async function POST(request) {
    try {
        const { inventory_items } = await request.json();
        
        if (!Array.isArray(inventory_items)) {
            return Response.json({ error: 'Expected an array of inventory items' }, { status: 400 });
        }
        ...
        if (inventory_items.length > 0) {
            ...
            for (const item of inventory_items) {
                ...
                if (cleaned && !seen.has(cleaned)) {
                    seen.add(cleaned);
                    rowsToInsert.push({ inventory_item: cleaned });
                }
            }
